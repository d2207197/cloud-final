...
uni2idx = json.load(open('web1t_unigrams_300000up.json'))
...
class MongoQuery():
    ...
    def rowkey(self):
        idxes = [ uni2idx[w] for w in self.text ]
        return ''.join([struct.pack(">I", idx) for idx in idxes])
    ...
    def column(self):
        ...
        # return map(lambda p: "sel" + ':' + str(p), self.col.poss)

        # text_length = str(len(self.text))
        # return ''.join(map(str, self.col.poss))
        # return str(self.col)
        return 'sel:' + str(self.col.next_pos) + '-' + ''.join(map(str, self.col.poss))

    def length(self):
        return self.col.next_pos

    def __str__(self):
        return '<r:\'{}\', t:{}, c:{}, f:{}>'.format(repr(self.rowkey()), self.text, self.column(), self.filters)
...
class Query():
    ...
    def __init__(self, toks):
        ...
        self._maybe = False
        for gram in toks:
            # print 'filter: ', POS, col.next_pos, filters
            # print self._querys
    ...
    def _insert_POS(self, POS):
        to_del = []
        # print 'add POS before: ', self._querys
        ...
        for i, (text, col, filters) in enumerate(self._querys):
            ...
            # print filters
            # print 'add POS mid: ', self._querys
        ...
        # print 'add POS after: ', self._querys
...
class HBaseNgram:
    ...
    def query(self, query, limit=None):
        ...
        querys = filter(lambda q: q.length() > 1, querys)
